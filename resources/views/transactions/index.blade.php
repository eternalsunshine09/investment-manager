        ...
        <div class="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-6">
            <form action="{{ route('transactions.index') }}" method="GET" class="flex flex-wrap gap-3 items-center">
                <select name="type" onchange="this.form.submit()"
                    class="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 font-bold outline-none">
                ...
                <a href="{{ route('transactions.index') }}"
                    class="text-red-500 font-bold text-sm hover:underline ml-2">Reset Filter ✕</a>
                @endif
            </form>
        </div>
